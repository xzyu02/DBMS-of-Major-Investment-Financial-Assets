# May not used, pending final decision
# This file is from thrid party website for time selection module in search.py
# source: https://blog.csdn.net/qq_44986363/article/details/124606342
import datetime
import calendar
...
class Calendar:
    ...
    def __place_widgets(self):
        ...
        xframe.pack(in_=self.G_Frame, side='bottom', pady=5)
        
        lbtn = ttk.Button(hframe, style='L.TButton', command=self._prev_month)
        lbtn.grid(in_=hframe, column=0, row=0, padx=5)
        rbtn = ttk.Button(hframe, style='R.TButton', command=self._next_month)
        rbtn.grid(in_=hframe, column=5, row=0, padx=5)
        #年下拉框
        ...
        ttk.Button(xframe, text = "确 定", width = 6, command = lambda: self._exit(True)).grid(row = 1, column = 0, sticky = 'ns', padx = 20)
        ttk.Button(xframe, text = "取 消", width = 6, command = self._exit).grid(row = 1, column = 4, sticky = 'ne', padx = 20)   
    ...
    def _main_judge(self):
        """判断窗口是否在最顶层"""
        try:
            if self.master.focus_displayof() == None or 'toplevel' not in str(self.master.focus_displayof()): self._exit()
    ...
    def selection(self):
        ...
        year, month = self._date.year, self._date.month
        print(year)
        return str(datetime(year, month, int(self._selection[0])))
