...
def show_assets():
    ...
    # Note words
    Label(show_assets, text="Find assets you want to add, then manually input back to previous page").pack(
        ipadx=10, ipady=10, expand=False)
    # Select Type combobox
    global asset_type
    asset_type = StringVar()
    combo1 = ttk.Combobox(show_assets, textvariable=asset_type, value=[
                          "Type of Asset is ...", "Commodity", "CryptoCurrency", "Foreign Currency", "Stock"], state='readonly')
    combo1.current(0)
...
# title
Label(watchlists, text="My Watchlist", font="bald", bg="grey",
      fg="white", width=300).pack(ipadx=10, ipady=10, expand=False)
# Note words
Label(watchlists, text="Click 'Add' to query the asset you want to add\n After find your assets, manually input in the chart below\n Click 'Exit' to close the program").pack(
    ipadx=10, ipady=10, expand=False)
...
list_chart = ttk.Treeview(watchlists, height=20,
